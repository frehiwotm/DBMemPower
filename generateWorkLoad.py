...
def executeInsert(connection, table, cols, row):
    to_db = [bytes(row[i], "utf-8"), range(len(cols))]
    qry =    "INSERT INTO " + table 
    qry += ' (' + ','.join(cols) + ') VALUES ('  + ','.join(['?']*len(cols)) + ');'
    
    connection.execute(qry, to_db)
...
def setupContent(c):    
    ...
    #                 to_db = [bytes(row[i], "utf-8"), range(16)]
    #
